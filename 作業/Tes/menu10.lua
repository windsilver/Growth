...
local bg = display.newImage("bg/bg10.jpg",centerX,centerY)
        bg.width = centerX*2
        bg.height = centerX*2
...
function onTouch(event)
        if event.phase == "began" then
            if event.target.name == "up1" then
				composer.gotoScene("menu9",{effect = "fromLeft",time = 500})
                    
            end 
            if event.target.name == "down1" then
				composer.gotoScene("menu11",{effect = "fromLeft",time = 500})       
            end 
        end
    end
