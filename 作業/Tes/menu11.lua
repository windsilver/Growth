...
local bg = display.newImage("bg/bg11.jpg",centerX,centerY)
        bg.width = centerX*2
        bg.height = centerX*2
...
function onTouch(event)
        if event.phase == "began" then
            if event.target.name == "up1" then
				composer.gotoScene("menu10",{effect = "fromTop",time = 500})
                    
            end 
            if event.target.name == "down1" then
				composer.gotoScene("menu12",{effect = "fromTop",time = 500})       
            end 
        end
    end
