...
local bg = display.newImage("bg/bg3.jpg",centerX,centerY)
        bg.width = centerX*2
        bg.height = centerX*2
...
function onTouch(event)
        if event.phase == "began" then
            if event.target.name == "up1" then
				composer.gotoScene("menu2",{effect = "zoomOutIn",time = 500})
                    
            end 
            if event.target.name == "down1" then
				composer.gotoScene("menu4",{effect = "zoomOutIn",time = 500})       
            end 
        end
    end
